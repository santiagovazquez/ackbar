...
import type { CardInput, ListingKind } from "@swu/shared";
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const listing = await createListing(
        { kind, title, imageUrl: imageUrl || undefined, items },
        token,
      );
        ...
        <div className="item-row" key={index}>
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) =>
                update(index, {
                  unitPriceCents: e.target.value ? Math.round(Number(e.target.value) * 100) : null,
                })
              }
            />
          </label>
