...
To copy one local publication into another database, run `pnpm --filter @swu/api db:import-listing -- --source local.db --listing <id> --owner-email <email>`. This performs a dry run and resolves the target owner by email. Review the output and repeat with `--confirm` to write the publication, its items, and its images atomically. Run it with `DATABASE_URL` and, when needed, `DATABASE_AUTH_TOKEN` configured for the target database.
